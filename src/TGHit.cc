...
G4Allocator<TGHit> TGHitAllocator;

TGHit::TGHit()
{pLogV=0;}

TGHit::TGHit(G4LogicalVolume* logVol,G4int absNo) 
...
const TGHit& TGHit::operator=(const TGHit &right)
{
  AbsID = right.AbsID;
  trackID = right.trackID;
  status = right.status;
...
int TGHit::operator==(const TGHit &right) const
{
  return (this==&right) ? 1 : 0;
}


std::map<G4String,G4AttDef> TGHit::fAttDefs;
...
const std::map<G4String,G4AttDef>* TGHit::GetAttDefs() const
{
...
std::vector<G4AttValue>* TGHit::CreateAttValues() const
{
  // Create expendable G4AttsValues for picking...
  std::vector<G4AttValue>* attValues = new std::vector<G4AttValue>;
  attValues->push_back
    (G4AttValue("HitType","TGHit",""));
  attValues->push_back
