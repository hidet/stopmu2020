...
class DEGHit : public G4VHit
{
public:
  
  DEGHit();
  DEGHit(G4LogicalVolume* logVol, G4int absNo);
  ~DEGHit();
  DEGHit(const DEGHit &right);
  const DEGHit& operator=(const DEGHit &right);
  G4int operator==(const DEGHit &right) const;
...
typedef G4THitsCollection<DEGHit> DEGHitsCollection;

extern G4Allocator<DEGHit> DEGHitAllocator;

inline void* DEGHit::operator new(size_t)
{
  void* aHit;
  aHit = (void*)DEGHitAllocator.MallocSingle();
  return aHit;
}

inline void DEGHit::operator delete(void* aHit)
{
  DEGHitAllocator.FreeSingle((DEGHit*) aHit);
}
