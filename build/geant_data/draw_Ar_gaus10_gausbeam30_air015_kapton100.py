...
parser.add_option('--p1',  dest='p1', action="store",type=str, help='ArGas atm 010',default=None)
parser.add_option('--p2',  dest='p2', action="store",type=str, help='ArGas atm 0.10',default=None)
options,args = parser.parse_args()

#p1="010"# string without dot 010
#p2="0.10"# string with dot 0.10
p1=options.p1
p2=options.p2
...
fnames = ["mu-_mom%d_gaus10_Ar%satm_gausbeam30_015air_100kapton"%(int(mom),p1) for mom in Mom]
Mom=Mom/10.
...
legend.Draw("same")
c1.SaveAs("mu-_stoppos_momdep_Ar%satm_gausbeam30_air015_kapron100_momgaus10.pdf"%(p1))
...
for i,(f,fname,htg) in enumerate(zip(fs,fnames,htgs)):
    ...
    if i==0:
        ...
        htg.SetTitle("mu- stopped position on ArGas target / 10k")
...
legend.Draw("same")
c2.SaveAs("mu-_target_stoppos_momdep_Ar%satm_gausbeam30_air015_kapton100_momgaus10.pdf"%(p1))
...
plt.ylabel("# of stopped $\mu^-$ on Ar %s atm target [/sec]"%(p2))
...
plt.tight_layout()
plt.savefig("Ar%satm_tesaccept_stopmu_per_sec_air015_kapton100_momgaus10_gausbeam30.pdf"%(p1))
...
plt.xlabel("Momentum [MeV/c]")
plt.ylabel("# of stopped $\mu^-$ on Ar %s atm target [/sec]"%(p2))
plt.title("stopped $\mu^-$ on Ar %s atm target"%(p2))
...
plt.tight_layout()
plt.savefig("Ar%satm_stopmu_per_sec_air015_kapton100_momgaus10.pdf"%(p1))
...
plt.legend()
plt.tight_layout()
plt.savefig("Activity_Ar%satm_stopmu_Bq_150kW_air015_kapton100_momgaus10_gausbeam30.pdf"%(p1))
...
plt.legend()
plt.tight_layout()
plt.savefig("Activity_Ar%satm_stopmu_Bq_550kW_air015_kapton100_momgaus10_gausbeam30.pdf"%(p1))
