...
parser.add_option('--p1',  dest='p1', action="store",type=str, help='N2Gas atm 010',default=None)
parser.add_option('--p2',  dest='p2', action="store",type=str, help='N2Gas atm 0.10',default=None)
...
#fnames = ["mu-_mom%d_gaus3_N%satm_gausbeam30_120air"%(int(mom),p1) for mom in Mom]
fnames = ["mu-_mom%d_gaus10_N%satm_gausbeam30_132air"%(int(mom),p1) for mom in Mom]
Mom=Mom/10.
...
legend.Draw("same")
c1.SaveAs("mu-_stoppos_momdep_N%satm_gausbeam30_air_momgaus10.pdf"%(p1))
...
for i,(f,fname,htg) in enumerate(zip(fs,fnames,htgs)):
    ...
    if i==0:
        ...
        htg.SetTitle("mu- stopped position on N2Gas target / 10k")
...
legend.Draw("same")
c2.SaveAs("mu-_target_stoppos_momdep_N%satm_gausbeam30_air_momgaus10.pdf"%(p1))
...
plt.ylabel("# of stopped $\mu^-$ on N2 %s atm target [/sec]"%(p2))
...
plt.tight_layout()
plt.savefig("N%satm_tesaccept_stopmu_per_sec_air_momgaus10_gausbeam30.pdf"%(p1))
...
plt.xlabel("Momentum [MeV/c]")
plt.ylabel("# of stopped $\mu^-$ on N2 %s atm target [/sec]"%(p2))
plt.title("stopped $\mu^-$ on N2 %s atm target"%(p2))
...
plt.tight_layout()
plt.savefig("N%satm_stopmu_per_sec_air_momgaus10.pdf"%(p1))
...
plt.legend()
plt.tight_layout()
plt.savefig("Activity_N%satm_stopmu_Bq_150kW_air_momgaus10_gausbeam30.pdf"%(p1))
...
plt.legend()
plt.tight_layout()
plt.savefig("Activity_N%satm_stopmu_Bq_550kW_air_momgaus10_gausbeam30.pdf"%(p1))
