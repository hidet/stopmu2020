...
#fnames = ["mu-_mom%d_bite5_Ne01atm_gausbeam"%(int(mom)) for mom in Mom]
#fnames = ["mu-_mom%d_bite5_Ne01atm_gausbeam_120air"%(int(mom)) for mom in Mom]
#fnames = ["mu-_mom%d_gaus3_Ne01atm_gausbeam_120air"%(int(mom)) for mom in Mom]
fnames = ["mu-_mom%d_gaus3_Ne010atm_gausbeam_120air"%(int(mom)) for mom in Mom]
...
legend.Draw("same")
c1.SaveAs("mu-_stoppos_momdep_Ne010atm_gausbeam_air_momgaus.pdf")
...
for i,(f,fname,htg) in enumerate(zip(fs,fnames,htgs)):
    ...
    if i==0:
        ...
        htg.GetYaxis().SetRangeUser(0,500)
        htg.Draw("hist")
    else:
        htg.Draw("histsame")
    legend.AddEntry(htg.GetName(), fname+" stpr=%.2f"%(stop_rate[i]), "l");
legend.Draw("same")
c2.SaveAs("mu-_target_stoppos_momdep_Ne010atm_gausbeam_air_momgaus.pdf")
...
plt.ylabel("# of stopped $\mu^-$ on Ne 0.10 atm target [/sec]")
plt.title("stopped $\mu^-$ within TES position +- 5 cm")
plt.ylim(-1000.,1e4)
plt.legend()
plt.tight_layout()
plt.savefig("Ne010atm_tesaccept_stopmu_per_sec_air_momgaus.pdf")
...
plt.xlabel("Momentum [MeV/c]")
plt.ylabel("# of stopped $\mu^-$ on Ne 0.10 atm target [/sec]")
plt.title("stopped $\mu^-$ on Ne 0.10 atm target")
plt.legend()
plt.tight_layout()
plt.savefig("Ne010atm_stopmu_per_sec_air_momgaus.pdf")
...
plt.legend()
plt.tight_layout()
plt.savefig("Activity_Ne010atm_stopmu_Bq_150kW_air_momgaus.pdf")
...
plt.legend()
plt.tight_layout()
plt.savefig("Activity_Ne010atm_stopmu_Bq_550kW_air_momgaus.pdf")
